...
AMD responses are streamed to keep long generations active. The primary and fallback share one 660-second budget for retries and stream progress checks. A request's socket timeout is at most 600 seconds and is reduced to the remaining shared budget when it starts. The workflow has a hard 13-minute limit. GitHub requests have a 30-second timeout. Interrupted or incomplete streams never produce a review, and network timeouts are reported without printing credentials or response bodies. Once a stream starts, that model request is not retried automatically.
