...
AMD's shared model service offers free APIs; availability and account limits come from the provider. Each model allows up to three attempts for HTTP `429` or transient `5xx` errors, with bounded backoff. After the primary model remains unavailable, the configured fallback may be attempted once, using the same key and remaining time budget. A network failure or AMD's observed HTTP 200 SSE error envelope with both `error.type` and `error.code` set to `upstream_error` can also trigger fallback. Any partial primary output is discarded, and error messages and response bodies are never logged. Other malformed, empty, or incomplete responses and other HTTP errors cannot trigger fallback. If no time remains or the fallback also fails, the workflow fails visibly without posting a review. It never switches back to the primary, changes credentials, or selects another provider.
